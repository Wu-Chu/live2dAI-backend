...
from app.schemas.common import PaginatedList, Pagination
...
class ActiveRecordMixin:
    ...
    @classmethod
    async def all_by_ids(cls, session: AsyncSession, ids: List[int]):
        """
        Return all objects with the given ids.
    ...
    @classmethod
    async def all_by_field_in(cls, session: AsyncSession, field: str, values: List[Any]):
        """
        Return all objects with the given field and values using IN operator.
        Return an empty list if not found.
        """
        if not values:
            return []
        ...
        statement = select(cls).where(col(getattr(cls, field)).in_(values))
        result = await session.exec(statement)
        return result.all()
    ...
    async def paginated_by_query(
        ...
        Args:
            ...
            distinct_by (str): If not None, removes duplicate records based on the field.
                Default is False.
        ...
        if distinct_by:
            # 使用子查询来实现基于特定字段的去重
            subquery = select(func.min(cls.id).label('min_id')).group_by(getattr(cls, distinct_by)).subquery()
            statement = statement.where(cls.id.in_(select(subquery.c.min_id)))
        if fields:
            conditions = [
        ...
        if distinct_by:
            # 计算去重后的记录数，使用与主查询相同的子查询逻辑
            count_subquery = select(func.min(cls.id).label('min_id')).group_by(getattr(cls, distinct_by))
            ...
            if fuzzy_fields:
                ...
                count_subquery = count_subquery.where(or_(*fuzzy_conditions))
            
            if extra_conditions:
                count_subquery = count_subquery.where(and_(*extra_conditions))
        ...
        if filter:
            ...
            for key, value in filter_dict.items():
                if isinstance(value, list):
                    statement = statement.where(col(getattr(cls, key)).in_(value))
                    count_statement = count_statement.where(col(getattr(cls, key)).in_(value))
                elif isinstance(value, dict):
                    if "FUZZY" in value:
                        statement = statement.where(col(getattr(cls, key)).like(f"%{value['FUZZY']}%"))
                        count_statement = count_statement.where(col(getattr(cls, key)).like(f"%{value['FUZZY']}%"))
                    elif "MULTI_EQUAL" in value:
                        statement = statement.where(col(getattr(cls, key)).in_(value["MULTI_EQUAL"]))
                        count_statement = count_statement.where(col(getattr(cls, key)).in_(value["MULTI_EQUAL"]))
                    elif "MULTI_RANGE" in value:
                        range_conditions = []
                        for range in value["MULTI_RANGE"]:
                            range_conditions.append(and_(
                                col(getattr(cls, key)) >= range[0],
                                col(getattr(cls, key)) < range[1],
                            ))
                        statement = statement.where(or_(*range_conditions))
                        count_statement = count_statement.where(or_(*range_conditions))
                else:
    ...
    @classmethod
    async def delete_batch(cls, session: AsyncSession, to_delete_list: List[SQLModel]):
        for obj in to_delete_list:
            await obj.delete(session)
            # await cls._publish_event(EventType.DELETED, obj)
    ...
    #             cls.__name__.lower(), Event(type=event_type, data=data,data_type=cls.__name__.lower())
    #         )
    ...
    #         for item in items:
    #             yield Event(type=EventType.CREATED, data=item, data_type=cls.__name__.lower())
    ...
    #     subscriber = await event_bus.subscribe(cls.__name__.lower())
    ...
    #                     >= heartbeat_interval
    #                 ):
    #                     yield Event(type=EventType.HEARTBEAT, data=None, data_type=cls.__name__.lower())
    #                     last_event_time = datetime.now(timezone.utc)
    #     finally:
